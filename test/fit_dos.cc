    ...
    try
    {
        ...
        for ( Index i = 0; i < nSimulations; ++i )
        {
            try    // Exception handling inside parallel region.
            {
                ...
                #pragma omp critical
                std::cout << "Performing simulation No. " << model.params().simulationNo() << " (fitting)..." << std::endl;
                ...
                model.fit(config, input_experim, output_directory, output_plot_subdir, output_filename);
