    ...
    try
    {
        ...
        for ( Index i = 0; i < nSimulations; ++i )
        {
            ...
            for ( Index j = 0; j < iterationsNo; ++j )
            {
                ...
                if ( j > 0 )
                {
                    // Sigma can't be < 0. If so, let it be std::numeric_limits<Real>::digits10
                    VectorXr temp1 = VectorXr::LinSpaced(nSplits, std::max(sigma(minimum) - negative_shift, std::numeric_limits<Real>::digits10 * KB_T), sigma(minimum));
