...
VectorXr GaussianCharge::dcharge(const VectorXr & phi)
{
  ...
  for ( Index i = 0; i < phi.size(); ++i )
    {
      dcharge(i) = - Q * dn_approx(phi(i), params_.N0_, params_.sigma_);
      
      if ( params_.N0_2_ > 0.0 )
        {
          dcharge(i) += - Q * dn_approx(phi(i) + params_.shift_2_, params_.N0_2_, params_.sigma_2_);
        }
        
      if ( params_.N0_3_ > 0.0 )
        {
          dcharge(i) += - Q * dn_approx(phi(i) + params_.shift_3_, params_.N0_3_, params_.sigma_3_);
        }
        
      if ( params_.N0_4_ > 0.0 )
        {
          dcharge(i) += - Q * dn_approx(phi(i) + params_.shift_4_, params_.N0_4_, params_.sigma_4_);
        }
...
ExponentialCharge::ExponentialCharge(const ParamList & params, const QuadratureRule & rule)
  : Charge(params, rule) {}

Real ExponentialCharge::n_approx(const Real & phi, const Real & N0, const Real & lambda) const
{
  Real n = 0.0;
...
Real ExponentialCharge::dn_approx(const Real & phi, const Real & N0, const Real & lambda) const
{
  Real dn = 0.0;
...
VectorXr ExponentialCharge::charge(const VectorXr & phi)
{
  ...
  for ( Index i = 0; i < phi.size(); ++i )
    {
      charge(i) = - Q * n_approx(phi(i), params_.N0_exp_, params_.lambda_exp_);
    }
...
VectorXr ExponentialCharge::dcharge(const VectorXr & phi)
{
  ...
  for ( Index i = 0; i < phi.size(); ++i )
    {
      dcharge(i) = - Q * dn_approx(phi(i), params_.N0_exp_, params_.lambda_exp_);
    }
